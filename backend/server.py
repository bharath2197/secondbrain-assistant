...
# Your actual Supabase tables
TBL_PROFILE = "user_profile"
TBL_CHAT = "chat_messages"
TBL_KB = "kb_entries"
TBL_REMINDERS = "reminders"
...
allow_origins = [o.strip() for o in cors_raw.split(",") if o.strip()] if cors_raw else ["*"]
...
REMINDER RULES:
- The ONLY truly required field is due_datetime_local.
- If the user gives a date/time but no explicit task/subject, set title to "Reminder" and create immediately. Do NOT ask for title.
...
async def get_profile(request: Request):
    ...
    rows = await db.select(TBL_PROFILE, {"user_id": f"eq.{uid}"})
...
async def upsert_profile(body: ProfileUpdate, request: Request):
    ...
    data = {"user_id": uid, "user_email": email, "timezone": body.timezone}
    result = await db.upsert(TBL_PROFILE, data, on_conflict="user_id")
...
async def chat_endpoint(body: ChatRequest, request: Request):
    ...
    profiles = await db.select(TBL_PROFILE, {"user_id": f"eq.{uid}"})
    user_tz = profiles[0].get("timezone") if profiles else "UTC"
    if not user_tz:
        user_tz = "UTC"

    recent = await db.select(TBL_CHAT, {
        "user_id": f"eq.{uid}",
    ...
    await db.insert(TBL_CHAT, {"user_id": uid, "role": "user", "content": body.message})
    ...
    async def _save_kb(kb_data):
        ...
        rows = await db.insert(TBL_KB, entry)
    ...
    async def _save_reminder(rem_data):
        ...
        rows = await db.insert(TBL_REMINDERS, rdata)
        ...
        try:
            ...
            if dt_local:
                abs_str = dt_local.strftime("%b %d, %Y at %I:%M %p").replace(" 0", " ").lstrip("0")
    ...
    await db.insert(TBL_CHAT, {"user_id": uid, "role": "assistant", "content": response_text})
...
async def get_messages(request: Request, limit: int = 200):
    db = get_db(request)
    uid = get_user_id(request)
    return await db.select(TBL_CHAT, {
        "user_id": f"eq.{uid}",
...
async def get_reminders(request: Request, status: str = None):
    ...
    return await db.select(TBL_REMINDERS, params)
...
async def update_reminder(reminder_id: str, body: ReminderStatusUpdate, request: Request):
    db = get_db(request)
    uid = get_user_id(request)
    result = await db.update(TBL_REMINDERS, {"status": body.status}, {"id": reminder_id, "user_id": uid})
